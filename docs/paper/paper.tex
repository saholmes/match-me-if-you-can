...
and is exercised by 36 workspace tests including end\nobreakdash-to\nobreakdash-end
integration tests against an axum\,+\,SQLite server.  The
age\nobreakdash-range AIR is wired through to the upstream
\texttt{deep\_ali} backend (\texttt{AirType::AgeRange32}); the
server's \texttt{POST /register} endpoint emits a real
$\num{405524}$\nobreakdash-byte STARK proof which
\texttt{GET /verify/age/\textit{id}} subsequently verifies under
the canonical\nobreakdash-serialize wire format.  The country /
e\nobreakdash-mail dual\nobreakdash-hash AIR remains at the
application\nobreakdash-layer membership check pending the
first\nobreakdash-class \texttt{MerklePath} and \texttt{Sha3}
\texttt{AirType} variants discussed in
\S\ref{sec:eval-compute}\,/\,\S\ref{sec:discussion} — the bench
harness measures the prove + verify cost of that combination
exactly via the existing \texttt{PoseidonChain} and
\texttt{Sha256DsKsk} reference AIRs (Tables~\ref{tab:compute}\,/\,\ref{tab:wasm}).
