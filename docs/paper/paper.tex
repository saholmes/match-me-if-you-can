...
\paragraph{WASM feasibility (measured).}  We compile the same
bench harness to \texttt{wasm32-wasip1} and re\nobreakdash-run it
under wasmtime~36; the same prove + verify cycle is exercised, just
with the AOT\nobreakdash-compiled wasm engine in place of native
\texttt{cargo --release}.  Table~\ref{tab:wasm} reports the
delta.  The slowdown is consistent across the ladder: prove
runs at $\approx\!0.36\times$ native speed, verify at
$\approx\!0.50\times$.  Proof bytes are unchanged
(WebAssembly does not change the proof format).

\begin{table}[h]
  \centering
  \caption{Native (\texttt{cargo --release}) vs
    WebAssembly (\texttt{wasm32-wasip1} under
    wasmtime~36) wall\nobreakdash-clock timings, median of
    $10$ runs each, same Apple\nobreakdash-Silicon laptop.  The
    WASM column is the relevant one for a client\nobreakdash-side
    prover; the native column models a server\nobreakdash-side
    prover.}
  \label{tab:wasm}
  \begin{tabular}{l r r r r r r}
    \toprule
    \textbf{AIR} & $n_\mathrm{trace}$ &
      \multicolumn{2}{c}{\textbf{Prove (ms)}} &
      \multicolumn{2}{c}{\textbf{Verify (ms)}} &
      \textbf{WASM\,/\,native} \\
    \cmidrule(lr){3-4} \cmidrule(lr){5-6}
                 &       & native & WASM & native & WASM & prove\,$|$\,verify \\
    \midrule
    Fibonacci  &  $32$ &  $6.95$ &  $19.42$ & $1.24$ & $2.45$ & $2.79\times \mid 1.97\times$ \\
    Fibonacci  &  $64$ & $14.15$ &  $39.62$ & $1.34$ & $2.71$ & $2.80\times \mid 2.03\times$ \\
    Fibonacci  & $128$ & $28.84$ &  $78.77$ & $1.52$ & $3.04$ & $2.73\times \mid 2.00\times$ \\
    Fibonacci  & $256$ & $56.40$ & $155.56$ & $1.69$ & $3.39$ & $2.76\times \mid 2.01\times$ \\
    \midrule
    HashRollup &  $32$ &  $7.38$ &  $19.39$ & $1.20$ & $2.42$ & $2.63\times \mid 2.02\times$ \\
    HashRollup &  $64$ & $14.91$ &  $39.09$ & $1.37$ & $2.69$ & $2.62\times \mid 1.96\times$ \\
    HashRollup & $128$ & $30.73$ &  $78.85$ & $1.52$ & $3.03$ & $2.57\times \mid 1.99\times$ \\
    HashRollup & $256$ & $61.20$ & $158.54$ & $1.75$ & $3.39$ & $2.59\times \mid 1.94\times$ \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{End\nobreakdash-to\nobreakdash-end browser registration.}
Combining the per\nobreakdash-attribute measurements: two
$32$\nobreakdash-row prove operations in series give
$\sim\!\num{40}$\,ms of WebAssembly CPU time on the client; the
$\sim\!\num{541232}$\nobreakdash-byte (\num{528}\,KiB)
proof bundle takes $\sim\!\num{400}$\,ms to upload over a
$4$G connection ($\sim\!10$\,Mbps up); the server's native verify is
