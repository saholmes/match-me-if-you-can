...
Table~\ref{tab:compute} directly.  The country Merkle\nobreakdash-path
AIR runs at $n_\mathrm{trace} \leq 32$ for $|S| \leq 256$
permitted countries; bracketed by Fibonacci and HashRollup it
sits at $\approx\!\num{7}$\,ms native prove time.  Both
attributes contribute the byte\nobreakdash-identical
$\num{270576}$\nobreakdash-byte proof already used in
Table~\ref{tab:storage}, since proof size is determined by the
FRI\nobreakdash-query schedule and trace size, not constraint
count or trace width (rows for $w=2$/$k=1$/$k=2$/$k=3$ all
agree to the byte).
...
Combining the per\nobreakdash-attribute measurements at the V8
column of Table~\ref{tab:wasm}: two $32$\nobreakdash-row prove
operations in series give $\sim\!\num{35}$\,ms of WebAssembly
CPU time on the client; the
$\sim\!\num{541232}$\nobreakdash-byte (\num{528}\,KiB)
proof bundle takes $\sim\!\num{400}$\,ms to upload over a
$4$G connection ($\sim\!10$\,Mbps up); the server's native verify
is $\sim\!\num{2.5}$\,ms.  Total wall\nobreakdash-clock latency:
$\sim\!\num{440}$\,ms.  This is comfortably below the one\nobreakdash-second
perceptual threshold for an interactive form submission, and is
competitive with current PII\nobreakdash-only registration flows
that already take $1$\textendash$3$\,seconds for an
e\nobreakdash-mail verification round\nobreakdash-trip.  We treat
this as conclusive evidence for the WASM\nobreakdash-prover
deployability claim made in \S\ref{sec:design}.
...
first\nobreakdash-class \texttt{AgeRange32} variant to
\texttt{deep\_ali::AirType} for this paper, so the
age\nobreakdash-range / income\nobreakdash-bracket /
postcode\nobreakdash-prefix predicates are measured exactly in
Table~\ref{tab:compute} rather than via a structural proxy.
The wider Merkle\nobreakdash-path AIRs (country, e\nobreakdash-mail
domain) do not yet have a corresponding variant; we report
HashRollup as a structural upper bound for them.  Adding a
first\nobreakdash-class \texttt{MerklePath} variant is a focused
follow\nobreakdash-up — the constraint set is the standard
$\mathrm{node}_{i+1} = H(\mathit{lhs}_i \,\Vert\, \mathit{rhs}_i)$
transition over four trace columns and a Merkle\nobreakdash-root
boundary check — and we expect its prove time and proof bytes
to land within the bracket of Table~\ref{tab:compute} rather
than shift qualitatively, since proof size is FRI\nobreakdash-query\nobreakdash-dominated
(the table confirms identical proof sizes for $k \in \{1, 2, 3\}$
at the same $n_\mathrm{trace}$).
