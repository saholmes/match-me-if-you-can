...
\begin{verbatim}
  ...
  cargo run -p mmiyc-bench -- storage     --proof-bytes 6500
...
proof storage assumes $6{,}500$ bytes per attribute, which is
conservative for a STARK proof of a $32$\nobreakdash-bit range
constraint at $128$\nobreakdash-bit security under
$\mathrm{BCH}(511, 259, t=30)$\nobreakdash-class
parameters~\cite{stark-stir,starks}.  Two attributes are proven
per user (age + country) plus a fixed $80$\nobreakdash-byte metadata
overhead (commitment + policy ID + user ID reference).
...
\begin{table}[h]
  ...
  \begin{tabular}{l r r r}
    ...
    \midrule
    PII   &      \num{595004} &       \num{59.5} & $1.0\times$ \\
    Proofs & \num{130800000} &   \num{13080.0}  & $\mathbf{220\times}$ \\
    \bottomrule
  \end{tabular}
\end{table}

The $220\!\times$ multiplier is the headline cost.  At commodity
SSD prices ($\sim\!\pounds 0.05$/GB/month) the absolute cost is
$\sim\!\pounds 0.008$ per user per year — negligible compared to
the regulatory cost analysed in \S\ref{sec:eval-regulatory}.  We do
not yet measure proof generation time empirically; published
STARK\nobreakdash-STIR figures for similarly\nobreakdash-sized AIRs
on commodity laptop hardware put the prove\nobreakdash-time at
$50$\textendash$200$\,ms per attribute and the verify\nobreakdash-time
at sub\nobreakdash-millisecond~\cite{stark-stir}.

\subsection{Compute cost}
\label{sec:eval-compute}

\textbf{Provisional figures from upstream STARK\nobreakdash-STIR
benchmarks.}  Empirical timings against our reference
implementation are pending the
\texttt{deep\_ali}\nobreakdash-integration follow\nobreakdash-up and
will appear in the final paper.  Indicative figures from the
\cite{stark-stir} prover at comparable
parameters: $\sim\!100$\,ms prove time per attribute on a
\texttt{cargo --release} build on Apple\nobreakdash-Silicon laptop
hardware; verify time below $1$\,ms; end\nobreakdash-to\nobreakdash-end
registration latency dominated by the proof\nobreakdash-generation
phase, with the database write (sqlx + sqlite/postgres) under
$5$\,ms.
...
parameters and Table~\ref{tab:sensitivity} sweeps over breach
probability.
...
\begin{table}[h]
  \centering
  \caption{Sensitivity to breach probability.  Annual savings scale
    linearly in $p$.}
  \label{tab:sensitivity}
  \begin{tabular}{r r r r}
    \toprule
    \textbf{$p$ (per year)} & \textbf{Annual PII (\pounds)} & \textbf{Annual Proofs (\pounds)} & \textbf{Savings (\pounds)} \\
    \midrule
    $0.01$ &  $3{,}814$ & $0$ &  $3{,}814$ \\
    $0.025$ &  $9{,}536$ & $0$ &  $9{,}536$ \\
    $0.05$ & $19{,}071$ & $0$ & $19{,}071$ \\
    $0.10$ & $38{,}142$ & $0$ & $38{,}142$ \\
    $0.25$ & $95{,}355$ & $0$ & $95{,}355$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\paragraph{Real STARK\nobreakdash-STIR proof emission.}  The
evaluation treats the per\nobreakdash-attribute proof byte count as a
parameter rather than a measurement, using a conservative
$6{,}500$\nobreakdash-byte estimate at $128$\nobreakdash-bit security.
The integration with the upstream
\texttt{deep\_ali}~\cite{stark-stir} prover is a focused
follow\nobreakdash-up; we expect the empirical proof size to land
within $\pm\!50\,\%$ of the assumed value, which would not change
any of the qualitative conclusions but will tighten the
storage\nobreakdash-cost confidence interval.
